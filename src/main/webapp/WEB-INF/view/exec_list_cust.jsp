		...
		<table class="table-default" border="1" align="center" width="80%">
			...
			<c:forEach var="cust" items="${custList}" varStatus="status">
				<tr align="center">
					...
					<td>${cust.address}</td>
					<td>${cust.age}</td>
					<td><a href="execviewacc.html?customerId=${cust.customerId}">View Account</a></td>
					<td><a href="updateCustomer.html?customerId=${cust.customerId}">Update Customer</a></td>
					<td>
						<c:if test="${boolList[status.index] }">
							<a href="delete-customer.html?customerId=${cust.customerId}" onclick="return confirm('Do you want to delete?')">Delete Customer</a>
						</c:if>
						<c:if test="${not boolList[status.index] }">
