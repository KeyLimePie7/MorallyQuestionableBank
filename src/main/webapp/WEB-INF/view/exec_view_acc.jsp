...
<body>
	...
	<div class="main-content">
		<h1 class="page-title" align="center">Account Management System</h1>
		<c:if test="${not empty accList}">
			<table class="table-default" border="1" align="center" width="90%">
				<tr>
					<th>Account Id</th>
					<th>Customer Id</th>
					<th>Account Type</th>
				...
				<c:forEach var="acc" items="${accList}">
					<tr>
						...
						<td>
						<c:if test="${not empty acc.lastTransactionDate}">
							${acc.lastTransactionDate}
						</c:if>
						<c:if test="${empty acc.lastTransactionDate}">

						</c:if>
						</td>
						<td align="center">
							<c:if test="${acc.balance == 0 }">
								<a href="exec_delete.html?accountId=${acc.accountId}" onclick="return confirm('Do you want to delete?')">Delete</a>
							</c:if>
							<c:if test="${acc.balance != 0 }">
		...
		<c:if test="${accList.size() < 1 }">
			<p><font color="red">Customer does not have any accounts!</font></p>
		</c:if>
		<c:if test="${accList.size() < 2 }">
			<h2 align= center ><a href="exec_create.html?customerId=${customerId}"><button class="submit-button">Create New Account</button></a></h2>
		</c:if>
	</div>
